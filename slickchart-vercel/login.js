...
export default async function handler(req, res) {
  ...
  if (email && dbEnabled()) {
    try {
      ...
      const rows = await q`SELECT id, name, email, pass_hash, verified, totp_secret, totp_enabled FROM providers WHERE email = ${email}`;
      if (rows.length && verifyPassword(password, rows[0].pass_hash)) {
        if (rows[0].totp_enabled) {
          const code = String(b.totpCode || '').trim();
          if (!code) { res.status(200).json({ requiresTotp: true }); return; } // password is right — just need the second factor next
          if (!verifyTotp(rows[0].totp_secret, code)) {
            await recordAttempt(q, email);
            res.status(401).json({ error: 'That authenticator code is incorrect or expired.', requiresTotp: true });
            return;
          }
        }
