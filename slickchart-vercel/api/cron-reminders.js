      ...
      if (apptList.length) {
        ...
        for (const a of apptList) {
          ...
          if (notif.appointmentReminder !== false && inMorning && apptL.date && apptL.date === tomorrow) {
            ...
            // (Pre-visit check-in auto-send now lives in the dedicated, provider-driven cron-checkins.js,
            // which reads the provider's Square appointments directly — more reliable than this
            // client-synced path. Kept here only as the day-before appointment reminder.)
          }
