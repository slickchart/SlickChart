// POST /api/square/invoice  { amount, title, name, email, customerId?, dueDate? }
// Ensures a Square customer, creates an order, creates + publishes an invoice (emailed).
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 1) resolve/create the Square customer (invoices email a specific customer)
    ...
    // 2) order
    const order = await sf('/v2/orders', { method: 'POST', body: { idempotency_key: 'sc-o-' + Date.now(), order: { location_id: locationId, customer_id: customerId, line_items: [{ name: String(b.title || 'Service'), quantity: '1', base_price_money: { amount, currency: 'USD' } }] } } });
    ...
    res.status(200).json({ ok: true, invoiceId: invoice.id, url: (pub.invoice && pub.invoice.public_url) || '' });
